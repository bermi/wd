...
/**
 * waitForDisplayed(using, value, timeout, cb) -> cb(err)
 */
webdriver.prototype.waitForVisible = function(using, value, timeout, cb) {
  var _this = this;
  var endTime = Date.now() + timeout;
  ...
  var poll = function(){
    _this.isDisplayedUsing(using, value, function(err, visible) {
      if (err) {
        return cb(err);
      }
      ...
      if (visible) {
        ...
        if (Date.now() > endTime) {
          cb(new Error("Element didn't become displayed"));
        } else {
          setTimeout(poll, 200);
        }
      }
...
_.each(elementFuncTypes, function(type) {
  ...
  /**
   * waitForDisplayedByClassName(value, timeout, cb) -> cb(err)
   * waitForDisplayedByCssSelector(value, timeout, cb) -> cb(err)
   * waitForDisplayedById(value, timeout, cb) -> cb(err)
   * waitForDisplayedByName(value, timeout, cb) -> cb(err)
   * waitForDisplayedByLinkText(value, timeout, cb) -> cb(err)
   * waitForDisplayedByPartialLinkText(value, timeout, cb) -> cb(err)
   * waitForDisplayedByTagName(value, timeout, cb) -> cb(err)
   * waitForDisplayedByXPath(value, timeout, cb) -> cb(err)
   * waitForDisplayedByCss(value, timeout, cb) -> cb(err)
   */
  webdriver.prototype['waitForDisplayed' + elFuncSuffix(type)] = function(value, timeout, cb) {
    webdriver.prototype.waitForDisplayed.apply(this, [elFuncFullType(type), value, timeout, cb]);
  };
...
webdriver.prototype.isDisplayedUsing = function (queryType, querySelector, cb) {
  this.elementIfExists(queryType, querySelector, function(err, element){
    if(err){
      return cb(err);
    }
